...
def evaluateViews(request):
    ...
    fraud_result = final_result[(final_result['Classification'] == 'Fraud') | (final_result['Classification'] == 'Legitimate')]
    total = fraud_result.shape[0]
    criteria_count = pd.DataFrame(fraud_result.iloc[:,4:].stack().reset_index(level=1, drop=True).rename('Decision Criteria'))
    affected = pd.DataFrame(criteria_count.groupby('Decision Criteria').size()).reset_index()
    affected.columns = ('Decision Criteria','Count')
    affected['Importance'] = round((affected['Count']/total*100).astype(float),2)
    importance_rank = affected.sort_values('Count',ascending= False).reset_index(drop=True)
    importance_rank = importance_rank[importance_rank['Decision Criteria'] != 'None'].head(10) 
